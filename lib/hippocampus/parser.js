define(function (require, exports, module) {
  ...
  function Defvar (tokens, file) {
    file = file || '(unknown file)';
    consumeType(file, tokens, 'OPEN_PAREN');
    var lastToken = consumeSymbol(file, tokens, 'defvar');
    this.symbol = new Symbol(tokens, file);
    this.value = new Expression(tokens, file);
    consumeType(file, tokens, 'CLOSE_PAREN');
  }

  Defvar.prototype.compile =
    function Defvar_compile (inExpression, indent, indentStep, sourceMap) {
      if ( inExpression ) {
        throw new errors.SyntaxError(null, this.symbol.line, this.symbol.column);
      } else {
        return indentTo(indent) + 'var '
          + this.symbol.compile(false, indent, indentStep, sourceMap).trim()
          + ' = ' + this.value.compile(true, indent, indentStep, sourceMap).trim()
          + ';';
      }
    };
  ...
  Application.prototype.compile =
    function Application_compile (inExpression, indent, indentStep, sourceMap) {
      ...
      default:
        return indentTo(indent) + this.symbol.compile(true, indent, indentStep, sourceMap).trim()
          + '(' + compileParameterList(this.params, indent, indentStep, sourceMap) + ')'
          + (inExpression ? '' : ';');
      }
    };
  ...
  Object_.prototype.compile =
    function Object_compile (inExpression, indent, indentStep, sourceMap) {
      // TODO: allow forms which aren't a keyword to be evaluated before they
      // become keys.
      var pieces = [];
      pieces.push(indentTo(indent) + '{');
      pieces.push.apply(pieces, this.properties.map(function (p, i) {
        var isLast = (i === this.properties.length - 1);
        return indentTo(indent + indentStep)
          + p.symbol.compile(true, indent + indentStep, indentStep, sourceMap).trim().replace(/^\s*(\.)/, '')
          + ': ' + p.value.compile(true, indent + indentStep, indentStep, sourceMap).trim()
          + (isLast ? '' : ',' );
      }, this));
      pieces.push(indentTo(indent) + '}');
      return pieces.join('\n');
  ...
  List.prototype.compile =
    function List_compile (inExpression, indent, indentStep, sourceMap) {
      ...
      if ( this.elements.length > 5 ) {
        pieces.push.apply(pieces, this.elements.map(function (e, i) {
          var isLast = (i === this.elements.length - 1);
          return e.compile(true, indent + indentStep, indentStep, sourceMap)
            + (isLast ? '' : ',');
        }));
        pieces.push(indentTo(indent) + ']' + (inExpression ? '' : ';'));
        return pieces.join('\n');
      } else {
        pieces.push.apply(pieces, this.elements.map(function (e, i) {
          var isLast = (i === this.elements.length - 1);
          return e.compile(true, indent + indentStep, indentStep, sourceMap).trim()
            + (isLast ? '' : ',');
        }, this));
        pieces.push(']' + (inExpression ? '' : ';'));
        return pieces.join(' ');
      }
    };
  ...
  function String_ (tokens, file) {
    file = file || '(unknown file)';
    this.value = consumeType(file, tokens, 'STRING').value;
  }
